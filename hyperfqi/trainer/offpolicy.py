...
from hyperfqi.data import Collector
from hyperfqi.policy import BasePolicy
from hyperfqi.trainer import gather_info, test_episode
from hyperfqi.utils import BaseLogger, LazyLogger, MovAvg, tqdm_config
...
def offpolicy_trainer(
    ...
    """
    start_epoch, env_step, env_episode, gradient_step = 0, 0, 0, 0
    if resume_from_log:
        start_epoch, env_step, env_episode, gradient_step = logger.restore_data()
    ...
    if test_collector is not None:
        ...
        test_result = test_episode(
            ...
            env_step, env_episode, reward_metric
        )
        best_epoch = start_epoch
        best_reward, best_reward_std = test_result["rew"], test_result["rew_std"]
        if save_fn:
            save_fn(policy, env_step)
    ...
    for epoch in range(1 + start_epoch, 1 + max_epoch):
        ...
        with tqdm.tqdm(
            ...
            while t.n < t.total:
                if train_fn:
                    train_fn(epoch, env_step, env_episode)
                random_action = (env_step < learning_start) and random_start
                result = train_collector.collect(
                    n_step=step_per_collect, n_episode=episode_per_collect, random=random_action, logger=logger
                )
                if result["n/ep"] > 0 and reward_metric:
                    rew = reward_metric(result["rews"])
                    result.update(rews=rew, rew=rew.mean(), rew_std=rew.std())
                env_step += int(result["n/st"])
                env_episode += int(result["n/ep"])
                ...
                if env_step >= learning_start:
                    if step_per_collect is not None:
                        update_repeat = update_per_step * result["n/st"]
                    elif episode_per_collect is not None:
                        update_repeat = update_per_step * result["n/ep"]
                    for _ in range(round(update_repeat)):
                        gradient_step += 1
                        ...
                        losses.update({'env_step': env_step})
        ...
        if test_collector is not None:
            test_result = test_episode(
                policy, test_collector, test_fn, epoch, episode_per_test, logger, env_step, env_episode,
                reward_metric
            )
            ...
            if verbose:
                print(
                    f"Epoch #{epoch}: test_reward: {rew:.6f}(mean) {rew_std:.6f}(std), best_rew"
                    f"ard: {best_reward:.6f}(mean) {best_reward_std:.6f}(std) in #{best_epoch}"
                )
