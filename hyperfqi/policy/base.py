...
from hyperfqi.data import Batch, ReplayBuffer, to_numpy, to_torch_as
...
class BasePolicy(ABC, nn.Module):
    ...
    * :meth:`~hyperfqi.policy.BasePolicy.__init__`: initialize the policy, including \
        coping the target network and so on;
    * :meth:`~hyperfqi.policy.BasePolicy.forward`: compute action with given \
        observation;
    * :meth:`~hyperfqi.policy.BasePolicy.process_fn`: pre-process data from the \
        replay buffer (this function can interact with replay buffer);
    * :meth:`~hyperfqi.policy.BasePolicy.learn`: update policy with a given batch of \
        data.
    * :meth:`~hyperfqi.policy.BasePolicy.post_process_fn`: update the replay buffer \
        from the learning process (e.g., prioritized replay buffer needs to update \
        the weight);
    * :meth:`~hyperfqi.policy.BasePolicy.update`: the main interface for training, \
