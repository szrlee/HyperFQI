...
from hyperfqi.env import BaseVectorEnv, DummyVectorEnv
from hyperfqi.policy import BasePolicy
from hyperfqi.utils import BaseLogger
...
class Collector(object):
    ...
    :param env: a ``gym.Env`` environment or an instance of the
        :class:`~hyperfqi.env.BaseVectorEnv` class.
    :param buffer: an instance of the :class:`~hyperfqi.data.ReplayBuffer` class.
    ...
    dict or a :class:`~hyperfqi.data.Batch` with the modified keys and values. Examples
    ...
    def __init__(
        ...
        ensemble_num: int = 0,
        ensemble_mask: str = "bi",
        ensemble_prob: float = 0.5,
        target_noise_dim: int = 0,
        target_noise_type: str = 'sphere',
    ) -> None:
        super().__init__()
        ...
        self.target_noise_index = target_noise_dim
        self.env = env
    ...
    def _gen_target_noise(self):
        if self.target_noise_type in ["sp"]:
            noise = np.random.randn(self.env_num, 1, self.target_noise_dim).astype(np.float32)
            noise /= np.linalg.norm(noise, axis=-1, keepdims=True)
        elif self.target_noise_type in ["ssp"]:
            noise = np.random.randn(self.env_num, 1, self.target_noise_dim).astype(np.float32)
            noise /= np.linalg.norm(noise, axis=-1, keepdims=True)
            noise *= np.sqrt(self.target_noise_dim)
        elif self.target_noise_type in ["bi"]:
            noise_shape = (self.env_num, 1, self.target_noise_dim)
            noise = np.random.binomial(1, 0.5, size=noise_shape).astype(np.float32) / 0.5
        elif self.target_noise_type in ["gs"]:
            noise = np.random.randn(self.env_num, 1, self.target_noise_dim).astype(np.float32)
        else:
    ...
    def _gen_ensemble_mask(self):
        mask_size = (self.env_num, self._ensemble_num)
        masks = np.random.binomial(1, self._ensemble_prob, size=mask_size)
        return masks
    ...
    def reset(self) -> None:
        ...
        self.data = Batch(
            ...
            target_noise={}, ensemble_mask={}, env_ids={},
        )
        self.reset_env()
        self.reset_buffer()
        self.reset_stat()
    ...
    def reset_env(self) -> None:
        ...
        if self.target_noise_dim:
            self.data.target_noise = self._gen_target_noise()
    ...
    def collect(
        ...
        while True:
            ...
            self.data.env_ids = ready_env_ids
            ...
            if random:
                ...
                if no_grad:
                    ...
                    result = self.policy(self.data, last_state)
                bi_done = result.get("bi_done", False)
            ...
            if self.target_noise_dim:
                self.data.update(target_noise=self._gen_target_noise())
            if self.preprocess_fn:
                self.data.update(
                    self.preprocess_fn(
