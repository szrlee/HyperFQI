...
class Net(nn.Module):
    ...
    :param bool dueling_param: whether to use dueling network to calculate Q
        ...
        class:`~hyperfqi.network.common.MLP`. Default to None.

    .. seealso::

        Please refer to :class:`~hyperfqi.network.common.MLP` for more
        detailed explanation on the usage of activation, norm_layer, etc.

        You can also refer to :class:`~hyperfqi.network.continuous.Actor`,
        :class:`~hyperfqi.network.continuous.Critic`, etc, to see how it's
        suggested be used.
    """
